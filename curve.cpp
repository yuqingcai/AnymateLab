...
// #define DRAW_GUIDELINE
    ...
    for (int i = 0; i < _shapes.size(); i ++) {
        ...
        batch->uploadStaticBuffer(_vectexBuffer0.get(), offset,
                                  size, vertices.data());
        offset += size;
    ...
    for (int i = 0; i < _shapes.size(); i ++) {
        ...
        std::vector<glm::vec3> vertices;
        for (glm::vec2 p : vertices2D) {
            vertices.push_back(glm::vec3(p.x, p.y, -0.1));
        }
...
#ifdef DRAW_GUIDELINE
    // update outline guide line VBO
    offset = 0;
    for (int i = 0; i < _shapes.size(); i ++) {
        std::vector<glm::vec2>& vertices2D = _shapes[i]->getGuideLineVertices();
        std::vector<glm::vec3> vertices;
        for (glm::vec2 p : vertices2D) {
            vertices.push_back(glm::vec3(p.x, p.y, 0.0));
        }
    ...
    offset = 0;
    for (int i = 0; i < _shapes.size(); i ++) {
        std::vector<glm::vec2>& vertices = _shapes[i]->getGuideLineVertices();
        const QRhiCommandBuffer::VertexInput inputBindings[] = {
...
Curve::Curve()
{
    ...
    //     glm::vec2(-100, 0.0),
    // });
    // polygon2->setPen(pen0);
    // polygon2->createVertices();
    ...
    //     glm::vec2(80.0, -40.0));
    // line4->setPen(pen0);
    // line4->createVertices();
    // _shapes.push_back(line4);

    // Vangoh::Squircles* squircles = new Vangoh::Squircles(
    //     glm::vec2(0.0, 0.0), 40.0, 60.0, 4.0);
    // squircles->setPen(pen0);
